...
L.Control.OSMGeocoder = L.Control.extend({
	...
	onAdd: function (map) {
		...
		//var form = this._form = L.DomUtil.create('form', className + '-form row');

        input_div = L.DomUtil.create('div', 'col s10');
        button_div = L.DomUtil.create('div', 'col s2');
        ...
        document.getElementById('searcher').appendChild(input_div);
        document.getElementById('searcher').appendChild(button_div);
        

		L.DomEvent.addListener(button, 'click', this._geocode, this);
        L.DomEvent.addListener(input, 'onkeypress', this._geocode_if_key, this);
    ...
    _geocode_if_key: function(event){
	...
	_expand: function () {
		L.DomUtil.addClass(this._container, 'leaflet-control-geocoder-expanded row');
	},

	_collapse: function () {
		this._container.className = this._container.className.replace(' leaflet-control-geocoder-expanded row', '');
	}
});
