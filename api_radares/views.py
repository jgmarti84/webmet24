...
class RadarView(viewsets.ReadOnlyModelViewSet):
    ...
    Si desea una lista, consulte la url: /api_radares/radares/return_codes

    """
    serializer_class = serializers.RadarSerializer

    @list_route(methods=['get'], url_path='return_codes')
    ...
    @list_route(methods=['get'], url_path='return_active_codes')
...
class MinRadarProductView(viewsets.ReadOnlyModelViewSet):
    ...
    Si desea una lista, consulte la url: /api_radares/radares_prod/return_keys

    """
    serializer_class = serializers.MinRadarProductSerializer

    @list_route(methods=['get'], url_path='return_keys')
    def return_codes(self, request, pk=None):
        """
...
class RadarImageView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        try:
            date_from = datetime.strptime(self.request.query_params.get('date_from', None), "%Y-%m-%d %H:%M")
            date_to = datetime.strptime(self.request.query_params.get('date_to', None), "%Y-%m-%d %H:%M")
        except:
            pass
        polarimetric_var = self.request.query_params.get('polarimetric_var', None)
        code_radar = self.request.query_params.get('code_radar', None)
        ...
        if (date_from and date_to) is not None:
            ...
            queryset = queryset.filter(date__range=(timezone.make_aware(date_from, timezone.get_default_timezone()),
                                                    timezone.make_aware(date_to, timezone.get_default_timezone())))
            print queryset
        return queryset
