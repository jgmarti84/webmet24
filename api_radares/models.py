...
POLARIMETRICS_VAR_DESCRIPTION = (('TH', 'TH: Explicacion de la variable'),
                                 ('TV', 'TV: Explicacion de la variable'),
                                 ('TDR', 'TDR: Explicacion de la variable'),
                                 ('PHIDP', 'PHIDP: Explicacion de la variable'),
                                 ('KDP', 'KDP: Explicacion de la variable'),
                                 ('RHOHV', 'RHOHV: Explicacion de la variable'),
                                 ('VRAD', 'VRAD: Explicacion de la variable'),
                                 ('WRAD', 'WRAD: Explicacion de la variable'),
                                 ('HID', 'HID: Explicacion de la variable'))
...
class Radar(models.Model):
    ...
    point1_lat = models.DecimalField('Latitud Norte', max_digits=14, decimal_places=10, default=0)
    point1_long = models.DecimalField('Longitud Este', max_digits=14, decimal_places=10, default=0)
    point2_lat = models.DecimalField('Latitud Sur', max_digits=14, decimal_places=10, default=0)
    point2_long = models.DecimalField('Longitud Oeste', max_digits=14, decimal_places=10, default=0)
    ...
    def get_extent(self):
        lat_max = self.center_lat + self.img_radio / 111.325
        lat_min = self.center_lat - self.img_radio / 111.325
        kmts_grado_by_la = cos(self.center_lat) * 111.325
        long_max = self.center_long + self.img_radio / kmts_grado_by_la
        long_min = self.center_long - self.img_radio / kmts_grado_by_la
...
class RadarImage(models.Model):
    ...
    sweep = models.DecimalField('Ángulo de la elevacion', max_digits=4, decimal_places=2, default=0)
...
class Reference(models.Model):
    product = models.ForeignKey(RadarProduct, related_name='references')
